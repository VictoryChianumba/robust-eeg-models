...
class AttackExplainers:
    def __init__(self, ATTR_MAX_N, N_STEPS_IG, ig, muV_grid,
        ...
        self.prenorm_std_np = prenorm_std_np
        self.median_std_pre = median_std_pre
    ...
    def eps_from_muV(self, mu_v):            # scalar ε in z-space
        return float(mu_v / self.median_std_pre)

    def eps_uV_per_channel(self, eps_z):     # per-channel μV implied by εᶻ
        return (eps_z * self.prenorm_std_np).tolist()

    def pick_subset(self, X_in, y_in, max_n=None):
        if max_n is None:
            max_n = self.ATTR_MAX_N
        if max_n and X_in.size(0) > max_n:
            return X_in[:max_n], y_in[:max_n]
        return X_in, y_in

    def attr_IG(self, X_in, y_in):
        Xi, yi = self.pick_subset(X_in, y_in)
    ...
    @property
    def EXPLAINERS(self):
        return {"IG": self.attr_IG}

    def run_linf_sweep(self, atk_name, eps_list_muV, steps=None,
                       alpha_rule=lambda e: e / 8, batch=128, seed=42,
                       lp_sigma_t=None, cap_idx=None, E_CLEAN=None):
        ...
        with torch.no_grad():
            num_classes = int(self.model(self.X[:1]).shape[-1])
            preds_clean = self.model(self.X).argmax(1).cpu()
            clean_acc = float((preds_clean == self.y.cpu()).float().mean().item())
            clean_acc_pc = self.per_class_acc(self.y.cpu(), preds_clean, num_classes)

        for mu_v in eps_list_muV:
            eps_z = self.eps_from_muV(mu_v)
            kwargs = {}
            if atk_name == "PGD":
            ...
            atk = ctor(self.model, eps=float(eps_z), **kwargs)

            preds_adv, snrs, l2_all, l2_succ, linf_all, boundary_hits = [], [], [], [], [], []

            for i in range(0, N, batch):
                Xi = self.X[i:i + batch].detach().clone().requires_grad_(True)
                yi = self.y[i:i + batch]
                xa = atk(Xi, yi).detach()
                ...
                if lp_sigma_t is not None:
                    delta = xa - Xi
                    delta = self.smooth_delta_gauss(delta, sigma_t=lp_sigma_t)
                    delta = delta.clamp(-eps_z, eps_z)
                    xa = Xi + delta

                xa = self.per_channel_clamp(xa, self.train_min_t, self.train_max_t)
                ...
                snrs.extend(self.snr_db(Xi, xa))

                bmask = ((xa <= (self.train_min_t + 1e-12)) |
                         (xa >= (self.train_max_t - 1e-12))).float()
                boundary_hits.append(float(bmask.mean().item()))

                # cache consistent adversarial subset
            ...
            adv_acc = float((preds_adv == self.y.cpu()).float().mean().item())
            ...
            row_core = {
                ...
                "eps_z": float(eps_z),
                "eps_uV_per_channel": self.eps_uV_per_channel(eps_z),
                ...
                "adv_acc_per_class": self.per_class_acc(self.y.cpu(), preds_adv, num_classes),
    ...
    def run_deepfool(self, batch=128, seed=42, cap_idx=None, E_CLEAN=None):
        atk = ta.DeepFool(self.model, steps=50)
        preds_adv, snrs, l2_all, l2_succ, linf_all, boundary_hits = [], [], [], [], [], []
        N = self.X.size(0)
